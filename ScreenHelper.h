...
uint16_t getBMPColor(const char *filename) {
  ...
  if (pixelsize != 24) {
    Serial.println("[WARNING]: getBMPColor: Image is not 24 bpp - " + String(pixelsize) + " - " + String(filename));
    return 0x0000;
  }
...
void reprintMainLabels() {
  if (isNeedRedrawMainInfo == true) {
    isNeedRedrawMainInfo = false;
    tft.setTextColor(TFT_RED, TFT_BLACK);
    tft.drawString(cpu.hardwareName, 80, 3);
    tft.drawString(gpu.hardwareName, 80, 63);
    tft.drawString(ssd.hardwareName, 80, 123);
    tft.setTextColor(TFT_WHITE, TFT_BLACK);

    tft.drawString("Temperature:", 70, 18);
    tft.drawString("       Load:", 70, 33);
    tft.drawString("      Power:", 70, 48);

    tft.drawString("Temperature:", 70, 78);
    tft.drawString("       Load:", 70, 93);
    tft.drawString("      Power:", 70, 108);

    tft.drawString("Temperature:", 70, 138);
    tft.drawString("   Activity:", 70, 153);
    tft.drawString(" Used Space:", 70, 168);

    tft.drawString("  Full size:", 70, 183);
    tft.drawString("Used memory:", 70, 198);
    tft.drawString("       Used:", 70, 213);
    tft.drawString("  Available:", 70, 228);
  }
}

void printCpuToTFT() {
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString(String(cpu.temperature) +" `C    ", 160, 18);
  tft.drawString(String(cpu.load) + " %    ", 160, 33);
  tft.drawString(String(cpu.power) + " W    ", 160, 48);
}

void printGpuToTFT() {
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString(String(gpu.temperature) + " `C    ", 160, 78);
  tft.drawString(String(gpu.load) + " %    ", 160, 93);
  tft.drawString(String(gpu.power) + " W    ", 160, 108);
}

void printSsdToTFT() {
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString(String(ssd.temperature) + " `C    ", 160, 138);
  tft.drawString(String(ssd.activity) + " %    ", 160, 153);
  tft.drawString(String(ssd.usedSpace) + " %    ", 160, 168);
}

void printMemoryToTFT() {
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString(String(memory.allMemory) + " Gb    ", 160, 183);
  tft.drawString(String(memory.usedPercentage) + " %    ", 160, 198);
  tft.drawString(String(memory.usedMemory) + " Gb    ", 160, 213);
  tft.drawString(String(memory.availableMemory) + " Gb    ", 160, 228);
}
